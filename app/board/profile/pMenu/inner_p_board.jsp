		...
		<div class="row gtr-uniform gtr-150">
			<c:forEach var="board" items="${boardList}" varStatus="status">
				<section class="col-4 col-6-large col-12-small">
					<div class="imgbox" style="background: #FFF;" onclick="getMyboard(${board.getPosition_serial()})">
						<c:choose>
							<c:when test="${srcList[status.index] == null}">
								<h2>${board.getBoard_title()}</h2>
								<script>
									$('.imgbox').css('border', '1px solid black');
								</script>
							</c:when>
							<c:otherwise>
								<img class="fImg" src="${srcList[status.index]}" alt=""/>
							</c:otherwise>
						</c:choose>
					</div>
				</section>
